    ...
    <div className="grid gap-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Başvuru no: {applicationDisplayNumber(app)}</CardDescription>
        </CardHeader>
        <CardContent>
        ...
        {isDoctor ? (
          <CardFooter className="border-t pt-6 flex-col items-start">
            <Button variant="outline" size="sm" onClick={openPacs}>
              PACS görüntüleyici
            </Button>
            {pacsUrl ? (
      ...
      {isNurse && isNurseReviewStatus(app.statusCode) ? (
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Sekreterya değerlendirmesi</CardTitle>
            <CardDescription>Onay veya red gerekçesi ile değerlendirin.</CardDescription>
          </CardHeader>
          ...
          <CardFooter className="flex flex-wrap gap-2 border-t pt-6">
            <Button disabled={busy} onClick={() => assess(true)}>
              Onayla
            </Button>
            <Button variant="destructive" disabled={busy} onClick={() => assess(false)}>
              Reddet
            </Button>
            <Button variant="secondary" disabled={busy} onClick={sendToDoctor}>
              Doktora yönlendir
            </Button>
          </CardFooter>
        </Card>
      ...
      {isDoctor && isDoctorEditableStatus(app.statusCode) ? (
        <>
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Rapor taslağı</CardTitle>
              <CardDescription>JSON formatında taslak kaydedin.</CardDescription>
            </CardHeader>
            <CardContent>
              <Textarea
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Başvuruyu sonuçlandır</CardTitle>
              <CardDescription>Nihai raporu JSON olarak gönderin.</CardDescription>
            </CardHeader>
            <CardContent>
              <Textarea
                className="min-h-[160px] font-mono text-xs"
            ...
            <CardFooter className="border-t pt-6">
              <Button disabled={busy} onClick={conclude}>
                Sonuçlandır
              </Button>
            </CardFooter>
          </Card>
