...
export function AgreementModal({ isOpen, onClose, onAccept, title, type }: AgreementModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="flex max-h-[85vh] max-w-[calc(100vw-2rem)] flex-col gap-0 overflow-hidden p-0 sm:max-w-2xl">
        <DialogHeader className="shrink-0 border-b px-4 py-4 text-left sm:px-6">
          <DialogTitle>{title}</DialogTitle>
          <DialogDescription className="sr-only">Sözleşme metnini okuyun ve onaylayın</DialogDescription>
        </DialogHeader>

        <div
          ref={scrollContainerRef}
          onScroll={handleScroll}
          className="min-h-0 flex-1 space-y-4 overflow-y-auto px-4 py-4 text-sm leading-relaxed text-muted-foreground sm:px-6"
        >
          {type === "terms" ? (
            <>
            ...
            <>
              ...
              <p>
                ...
                <br />- Hekimlerimiz tarafından tıbbi danışmanlık ve ikinci görüş hizmetinin yürütülmesi,
                <br />- Faturalandırma, ödeme tahsilatı ve iade süreçlerinin yönetilmesi,
                <br />- Kimlik doğrulama süreçlerinin işletilmesi,
                <br />- Güvenlik ve mevzuata uyum yükümlülüklerinin yerine getirilmesi amacıyla işlenir.
              </p>
        ...
        <DialogFooter className="shrink-0 flex-col gap-2 border-t bg-muted/30 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
          <p className="text-xs text-muted-foreground sm:mr-auto">
            {!hasScrolledToBottom
              ? "Lütfen onaylamak için metni sonuna kadar okuyun."
              : "Metni okudunuz, onaylayabilirsiniz."}
          </p>
          <div className="flex w-full gap-2 sm:w-auto">
            <Button variant="outline" size="sm" className="flex-1 sm:flex-none" onClick={onClose}>
              Kapat
            </Button>
            <Button
              variant="default"
