  ...
  return (
    <div className="min-h-svh flex flex-col bg-background text-foreground relative overflow-hidden pb-[env(safe-area-inset-bottom,0px)]">
      ...
      <header className="relative z-10 w-full max-w-5xl mx-auto px-4 py-4 sm:px-6 sm:py-6 flex items-center justify-between pt-[max(1rem,env(safe-area-inset-top,0px))]">
        ...
        <Badge
          variant="outline"
          className="px-3 py-1 text-xs border-primary/20 bg-primary/5 text-primary font-medium backdrop-blur-sm"
        >
          {badge} Portalı
        </Badge>
      </header>

      <main className="relative z-10 flex-1 flex flex-col justify-center px-4 py-6 sm:px-6 sm:py-8">
        <div className={cn("w-full mx-auto space-y-6", wide ? "max-w-2xl" : "max-w-md")}>
          {children}
        </div>
      </main>
