export const API = {
  auth: {
    login: "/api/v1/auth/login",
    refresh: "/api/v1/auth/refresh",
    agreements: "/api/v1/auth/agreements",
    registerInitiate: "/api/v1/auth/register/initiate",
    registerComplete: "/api/v1/auth/register/complete",
    forgotInitiate: "/api/v1/auth/password/forgot/initiate",
    forgotComplete: "/api/v1/auth/password/forgot/complete",
  },
  professions: (targetInstitution: number) =>
    `/api/v1/professions?targetInstitution=${targetInstitution}`,
  careProviders: (targetInstitution: number, professionCode: string) =>
    `/api/v1/care-providers?targetInstitution=${targetInstitution}&professionCode=${encodeURIComponent(professionCode)}`,
  applications: {
    create: "/api/v1/applications",
    mine: "/api/v1/applications/mine",
    doctorQueue: "/api/v1/applications/queue/doctor",
    doctorQueueStats: "/api/v1/applications/queue/doctor/stats",
    nurseQueue: "/api/v1/applications/queue/nurse",
    detail: (id: string) => `/api/v1/applications/${id}`,
    update: (id: string) => `/api/v1/applications/${id}`,
    preview: (id: string) => `/api/v1/applications/${id}/preview`,
    cancel: (id: string) => `/api/v1/applications/${id}`,
    attachments: (id: string) => `/api/v1/applications/${id}/attachments`,
    attachment: (id: string, attachmentId: string) =>
      `/api/v1/applications/${id}/attachments/${attachmentId}`,
    payment: (id: string) => `/api/v1/applications/${id}/payment`,
    paytrToken: (id: string) => `/api/v1/applications/${id}/payment/paytr/token`,
    paytrSimulate: (id: string) => `/api/v1/applications/${id}/payment/paytr/simulate`,
    report: (id: string) => `/api/v1/applications/${id}/report`,
    reportHtml: (id: string) => `/api/v1/applications/${id}/report/html`,
    reportUpdate: (id: string) => `/api/v1/applications/${id}/report`,
    assess: (id: string) => `/api/v1/applications/${id}/assess`,
    sendToDoctor: (id: string) => `/api/v1/applications/${id}/send-to-doctor`,
    reportDraft: (id: string) => `/api/v1/applications/${id}/report/draft`,
    reportPreview: (id: string) => `/api/v1/applications/${id}/report/preview`,
    conclude: (id: string) => `/api/v1/applications/${id}/conclude`,
    notes: (id: string) => `/api/v1/applications/${id}/notes`,
    messages: (id: string) => `/api/v1/applications/${id}/messages`,
    chatWs: "/api/v1/ws/chat",
  },
  erciyes: {
    pacsUrl: "/api/v1/integrations/erciyes/pacs-url",
  },
  admin: {
    applications: "/api/v1/admin/applications",
    applicationHistory: (id: string) => `/api/v1/admin/applications/${id}/history`,
    hospitals: "/api/v1/admin/hospitals",
    hospitalUpdate: (id: string) => `/api/v1/admin/hospitals/${id}`,
    doctors: "/api/v1/admin/doctors",
    doctorDetail: (id: string) => `/api/v1/admin/doctors/${id}`,
    doctorUpdate: (id: string) => `/api/v1/admin/doctors/${id}`,
    professions: "/api/v1/admin/professions",
    professionUpdate: (id: string) => `/api/v1/admin/professions/${id}`,
    createProfession: "/api/v1/admin/professions",
    payments: "/api/v1/admin/payments",
    paymentInvoice: (id: string) => `/api/v1/admin/payments/${id}/invoice`,
    refunds: "/api/v1/admin/refunds",
    notifications: "/api/v1/admin/notifications",
    notificationsExport: "/api/v1/admin/notifications/export",
    notificationDetail: (id: string) => `/api/v1/admin/notifications/${id}`,
    smsOtpLogs: "/api/v1/admin/sms-otp-logs",
    contactMessages: "/api/v1/admin/contact-messages",
    contactMessageUpdate: (id: string) => `/api/v1/admin/contact-messages/${id}`,
    accountingSettings: "/api/v1/admin/accounting/settings",
    accountingReport: "/api/v1/admin/accounting/report",
    reportsOverview: "/api/v1/admin/reports/overview",
    doctorEarnings: "/api/v1/admin/reports/doctor-earnings",
    doctorEarningsExport: "/api/v1/admin/reports/doctor-earnings/export",
    erciyesHealth: "/api/v1/admin/integrations/erciyes/health",
    titles: "/api/v1/admin/titles",
    titleUpdate: (id: string) => `/api/v1/admin/titles/${id}`,
  },
  public: {
    contact: "/api/v1/public/contact",
  },
} as const;
