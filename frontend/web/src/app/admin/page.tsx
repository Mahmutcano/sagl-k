...
export default function AdminDashboardPage() {
  ...
  const [statsApps, setStatsApps] = useState<AppRow[]>([]);
  ...
  function load(token: string) {
    return Promise.all([
      api<{ items?: AppRow[]; totalCount?: number } | AppRow[]>(
        `${API.admin.applications}?page=0&pageSize=500`,
        {},
        token
      ),
      api<{ items: PaymentRow[] }>(`${API.admin.payments}?pageSize=1000`, {}, token).catch(() => ({ items: [] })),
    ]).then(([a, p]) => {
      const payload = a as { items?: AppRow[]; totalCount?: number };
  ...
  const pagedApps = filteredApps.slice(page * PAGE_SIZE, (page + 1) * PAGE_SIZE);
  ...
  const pendingCount = statsApps.filter((a) =>
    a.statusCode === 0 || [1, 2, 4, 5, 10, 11].includes(a.statusCode)
  ).length;
    ...
    >
      ...
      <div className="mb-2 grid grid-cols-2 gap-3 lg:grid-cols-4 sm:gap-4 sm:mb-4">
        <Card className=" overflow-hidden rounded-xl bg-white/95 sm:rounded-2xl ">
          <CardContent className="flex items-center justify-between gap-2 p-3 sm:p-5">
            <div className="min-w-0 space-y-0.5 sm:space-y-1">
              <span className="block text-[10px] font-extrabold uppercase tracking-wider text-muted-foreground sm:text-[11px]">Toplam Ciro</span>
              <span className="block truncate text-base font-black text-foreground sm:text-xl">
                {totalEarnings.toLocaleString("tr-TR", { minimumFractionDigits: 2 })} ₺
              </span>
              <span className="hidden text-[10px] font-semibold text-muted-foreground sm:block">{paidCount} başarılı ödemeden</span>
            </div>
        ...
        <Card className=" overflow-hidden rounded-xl bg-white/95 sm:rounded-2xl ">
          <CardContent className="flex items-center justify-between gap-2 p-3 sm:p-5">
            <div className="min-w-0 space-y-0.5 sm:space-y-1">
              <span className="block text-[10px] font-extrabold uppercase tracking-wider text-muted-foreground sm:text-[11px]">Başarılı</span>
              <span className="block text-base font-black text-foreground sm:text-xl">{paidCount} Adet</span>
              <span className="hidden text-[10px] font-semibold text-muted-foreground sm:block">Hizmet bedeli tahsil edilen</span>
            </div>
        ...
        <Card className=" overflow-hidden rounded-xl bg-white/95 sm:rounded-2xl ">
          <CardContent className="flex items-center justify-between gap-2 p-3 sm:p-5">
            <div className="min-w-0 space-y-0.5 sm:space-y-1">
              <span className="block text-[10px] font-extrabold uppercase tracking-wider text-muted-foreground sm:text-[11px]">Bekleyen</span>
              <span className="block text-base font-black text-foreground sm:text-xl">{pendingCount} Adet</span>
              <span className="hidden text-[10px] font-semibold text-muted-foreground sm:block">İşlem bekleyen başvuru</span>
            </div>
        ...
        <Card className=" overflow-hidden rounded-xl bg-white/95 sm:rounded-2xl ">
          <CardContent className="flex items-center justify-between gap-2 p-3 sm:p-5">
            <div className="min-w-0 space-y-0.5 sm:space-y-1">
              <span className="block text-[10px] font-extrabold uppercase tracking-wider text-muted-foreground sm:text-[11px]">İade</span>
              <span className="block text-base font-black text-foreground sm:text-xl">{refundCount} Adet</span>
              <span className="hidden text-[10px] font-semibold text-muted-foreground sm:block">İadesi tamamlanmış</span>
            </div>
      ...
      {/* Premium Filter Area */}
      <Card className=" mb-2 overflow-hidden rounded-xl /80 bg-white/95 sm:mb-4 sm:rounded-2xl print:hidden">
      ...
      <Card className=" overflow-hidden rounded-xl /80 bg-white/95 sm:rounded-2xl">
        <CardHeader className="flex flex-row items-center justify-between gap-3 border-b bg-muted/40 px-3 py-3 sm:px-6 sm:py-5">
          <div className="flex min-w-0 items-center gap-2 sm:gap-3">
            <div className="hidden rounded-xl bg-primary/10 p-2 sm:block">
              <ClipboardList className="h-5 w-5 text-primary" />
            </div>
            <div className="min-w-0">
              <CardTitle className="flex flex-wrap items-center gap-2 text-sm font-bold text-foreground sm:text-base">
                Hasta Başvuruları
              ...
              <div className="divide-y divide-border md:hidden">
                {pagedApps.map((a) => (
                  <button
                    ...
                    className="flex w-full flex-col gap-2 px-3 py-3 text-left active:bg-muted"
                  >
                ...
                <Table>
                  <TableHeader className="bg-muted/30">
                    <TableRow className="hover:bg-transparent">
                      <TableHead className="h-10 px-4 text-[11px] font-bold uppercase tracking-wider text-muted-foreground lg:px-6">Hasta Adı</TableHead>
                      <TableHead className="h-10 px-4 text-[11px] font-bold uppercase tracking-wider text-muted-foreground lg:px-6">Başvuru Numarası</TableHead>
                      <TableHead className="h-10 px-4 text-[11px] font-bold uppercase tracking-wider text-muted-foreground lg:px-6">Referans No / ID</TableHead>
                      <TableHead className="h-10 px-4 text-[11px] font-bold uppercase tracking-wider text-muted-foreground lg:px-6">Oluşturulma</TableHead>
                      <TableHead className="h-10 px-4 text-[11px] font-bold uppercase tracking-wider text-muted-foreground lg:px-6">Durum</TableHead>
                      <TableHead className="h-10 px-4 text-right text-[11px] font-bold uppercase tracking-wider text-muted-foreground print:hidden lg:px-6"></TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {pagedApps.map((a) => (
                      <TableRow
                        key={a.applicationId}
                        onClick={() => router.push(ROUTES.admin.application(a.applicationId))}
                        className="group cursor-pointer border-b transition-all duration-150 last:border-0 hover:bg-muted/60"
                      >
                        <TableCell className="px-4 py-4 lg:px-6">
                          <div className="flex items-center gap-3">
                            ...
                            <Link
                              ...
                              className="font-sans font-semibold text-foreground underline-offset-4 transition-colors hover:underline group-hover:text-primary"
                            >
                              {a.patientName}
                            </Link>
                          </div>
                        ...
                        <TableCell className="px-4 py-4 lg:px-6">
                          <span className="inline-flex items-center gap-1 rounded border /60 bg-muted/80 px-2 py-0.5 font-mono text-xs font-semibold text-foreground">
                        ...
                        <TableCell className="px-4 py-4 lg:px-6">
                          {a.ecommerceNumber ? (
                            ...
                            <span className="rounded border /40 bg-muted px-2 py-0.5 font-mono text-xs text-muted-foreground">
                              {a.applicationId.slice(0, 8)}…
                            </span>
                          )}
                        </TableCell>
                        <TableCell className="px-4 py-4 font-sans text-xs font-medium text-muted-foreground lg:px-6">
                        ...
                        <TableCell className="px-4 py-4 text-right print:hidden lg:px-6">
                          <div className="flex justify-end opacity-0 transition-opacity duration-150 group-hover:opacity-100">
                            <span className="inline-flex items-center gap-0.5 font-sans text-xs font-bold text-primary transition-transform ">
                              Detay
      ...
      {filteredTotal > PAGE_SIZE ? (
        <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
          <p className="text-sm text-muted-foreground">
            Sayfa {page + 1} / {Math.max(1, Math.ceil(filteredTotal / PAGE_SIZE))} · {filteredTotal} kayıt
          </p>
          <div className="flex gap-2">
            <Button type="button" variant="outline" size="sm" disabled={page <= 0} onClick={() => setPage((p) => Math.max(0, p - 1))}>
              Önceki
            </Button>
            <Button type="button" variant="outline" size="sm" disabled={(page + 1) * PAGE_SIZE >= filteredTotal} onClick={() => setPage((p) => p + 1)}>
              Sonraki
            </Button>
          </div>
        </div>
